---
title: Linux Tool：Python3.6 安装使用
...
在 Python 3.6 以后的版本中，pip 以及部分网络连接需要使用到 SSL 协议，此时，如果 Python 在编译时没有使用 openssl 库，则在使用网络连接时会出现报错，比如造成无法使用 pip 安装的问题。
...
yum -y install tk-devel libffi-devel gcc-c++ perl
...
# 注意，当需要打包编译 py 文件（使用 pyinstaller）时，应当添加参数 --enable-shared
...
一般不建议这样处理，因为 Linux 中某些命令需要用到 python2，但你这样处理后，虽然直接使用 python 调用的会变成 python3，但某些命令可能会因为调用 python3 而出错。

```shell
# 若要将 python 命令指向 python3，可以先删除原链接再重新指向。
# 但这里不推荐这样做，因为 Linux 的部分包需要使用 python2
rm /usr/bin/python
...
同时，需要注意的是，在携带 `--enable-shared` 编译成功后，用户需要手动连接生成的 `libpython3.9.so.1.0` 文件，使用命令：
...
一般情况下，我们编写的脚本可以直接使用 `python test.py` 的命令来调用。但在某些时候，因为某些 package 或者 module 需要提前加载，因此如果直接使用 `python test.py` 我们的脚本将无法调用。

此时，我们需要通过指定 `-m` 参数，来让 Python 预先 import 你要的 package 或module 给你，然后再执行 script。
