...
- 第一个数字表示的是所有者（User）的权限，使用读、写、执行三个权限位的数字总和。比如 rwx ，也就是 4+2+1，即 7。
- 第二个数字表示的是用户组（Group）的权限，同样使用读、写、执行三个权限位的数字总和。比如 rw- ，也就是 4+2+0 ，即 6。
- 最后一个数字表示的是其它用户的权限，一样使用读、写、执行三个数字相加的总和。比如 r-x ，也就是 4+0+1 ，即 5。
